...
public class AlignmentMerger
{
	...
	{	
		...
		OWLOntology o_in = manager.loadOntologyFromOntologyDocument(f_in);
		...
		OWLOntology o_out;
		if(o_in.getOntologyID().getOntologyIRI().isEmpty())
			o_out = manager.createOntology(IRI.create(f_in));
		else
			o_out = manager.createOntology(o_in.getOntologyID().getOntologyIRI().get());
		...
		manager.saveOntology(o_out, manager.getOntologyFormat(o_in), IRI.create(new File(out)));
